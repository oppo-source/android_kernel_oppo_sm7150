...
__attribute__((weak)) void oppo_m1120_reconfig(struct oppo_dhall_chip *chip)
{
	return;
}

static int m1120_i2c_read_block(struct oppo_dhall_chip *chip, u8 addr, u8 *data,
				u8 len)
{
	u8 reg_addr = addr;
	int err = 0;
...
static int m1120_i2c_write_block(struct oppo_dhall_chip *chip, u8 addr,
				 u8 *data, u8 len)
{
	int err = 0;
	int idx = 0;
...
static void m1120_short_to_2byte(struct oppo_dhall_chip *chip, short x,
				 u8 *hbyte, u8 *lbyte)
{
...
static short m1120_2byte_to_short(struct oppo_dhall_chip *chip, u8 hbyte,
				  u8 lbyte)
{
	short x = 0;
...
static int m1120_enable_hidden(struct oppo_dhall_chip *chip, bool enable,
			       uint32_t ratio)
{
	int err = 0;
