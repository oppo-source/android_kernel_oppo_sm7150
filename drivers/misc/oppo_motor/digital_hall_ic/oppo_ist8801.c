...
__attribute__((weak)) void oppo_ist8801_reconfig(struct oppo_dhall_chip *chip)
{
	return;
}

static int probe_count = 0;
static int ist8801_i2c_read_block(struct oppo_dhall_chip *chip, u8 addr,
				  u8 *data, u8 len)
{
	u8 reg_addr = addr;
	int err = 0;
...
static int ist8801_i2c_write_block(struct oppo_dhall_chip *chip, u8 addr,
				   u8 *data, u8 len)
{
	int err = 0;
	int idx = 0;
...
static void ist8801_short_to_2byte(struct oppo_dhall_chip *chip, short x,
				   u8 *hbyte, u8 *lbyte)
{
	unsigned short temp;
...
static short ist8801_2byte_to_short(struct oppo_dhall_chip *chip, u8 hbyte,
				    u8 lbyte)
{
	short x = 0;
