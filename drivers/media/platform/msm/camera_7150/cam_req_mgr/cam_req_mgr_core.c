...
/* suraj.chennoji@Camera, 17022021, Corrected the CRM frame skip logic to fix alternate frame drop issue, QC Case: 05076623 */
...
/* suraj.chennoji@Camera, 17022021, Corrected the CRM frame skip logic to fix alternate frame drop issue, QC Case: 05076623 */
	...
	if ((link->sof_timestamp > sync_link->sof_timestamp) &&
		...
		if (req_id > sync_req_id) {
			...
			CAM_DBG(CAM_CRM,
				"sync link %x too quickly, skip next frame of sync link req:%lld sync reqid:%lld",
				sync_link->link_hdl, req_id, sync_req_id);
			link->sync_link_sof_skip = true;
		}
	}
#endif
