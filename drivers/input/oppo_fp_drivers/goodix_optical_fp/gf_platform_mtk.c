...
int gf_parse_pwr_list(struct gf_dev* gf_dev)
{
    ...
    unsigned delay_time_after_parse = 0;
    ...
    for_each_available_child_of_node(np, child) {
        ...
        /* get after-parsing powerlist delay time*/
        ret = of_property_read_u32(child, FP_POWER_DELAY_TIME_AFTER_PARSE, &delay_time_after_parse);
        if (ret) {
            pr_info("failed to request delay-time-after-parse, ret = %d\n", ret);
            delay_time_after_parse = 0;
            ret = 0;
        }
        child_node_index++;
        if (delay_time_after_parse > 0) {
            msleep(delay_time_after_parse);
        }
    }
